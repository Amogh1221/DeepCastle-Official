...
import { Cpu, Settings2, RefreshCw, TrendingUp, MessageSquare, Flag, RotateCcw, Lightbulb, ChevronRight } from "lucide-react";
...
export default function DeepcastleGrandmaster() {
      ...
      if (data.bestmove) {
        setGame((prevGame) => {
          ...
          try {
            ...
            try {
              // Standard Notation (Nf3, e4, etc.) vs Coordinate (e2e4)
          ...
          setBotMessage("Internal logic error preventing move execution.");
          return prevGame;
        });
      }
    } catch (error) {
      console.error(error);
      setBotMessage("Connection to HuggingFace brain failed. Please ensure the API is awake!");
    } finally {
      setThinking(false);
    }
  }, []);
  ...
  function makePlayerMove(from: string, to: string) {
    ...
    if (moveObj) {
      ...
      // Allow UI to instantly update before starting the engine fetch
      setTimeout(() => fetchMove(gameCopy.fen()), 100);
      return true;
    }
  ...
  function getMoveOptions(square: string) {
    // Determine legal moves for the clicked square
    ...
    const originalPiece = game.get(square as any);
  ...
  // react-chessboard triggers
  function onDrop(sourceSquare: string, targetSquare: string) {
    const success = makePlayerMove(sourceSquare, targetSquare);
    if (!success) {
      // Clear any pending highlights if drop fails
      setMoveFrom("");
      setOptionSquares({});
    }
    return success;
  ...
  function onSquareClick(square: string) {
    ...
    // If nothing currently selected, attempt to select something
    if (!moveFrom) {
    ...
    // Try applying a move from the previously selected square to this newly clicked square
    const moveSuccess = makePlayerMove(moveFrom, square);

    if (!moveSuccess) {
      // The user clicked an invalid destination. Either they clicked an empty square or their own piece to select it instead.
      const hasMoves = getMoveOptions(square);
      if (hasMoves) {
        setMoveFrom(square);
      } else {
        ...
        <div className="lg:col-span-6 flex flex-col gap-4">
          ...
          <div className="flex gap-4 items-stretch aspect-square lg:aspect-auto">
            ...
            <div className="flex-1 bg-[#262421] p-3 rounded-lg shadow-2xl relative border-2 border-[#3d3a36]">
               {/* Bypass React-Chessboard TS typing since their types constantly desync with React 19 */}
               <Chessboard 
                 {...({
                   position: game.fen(),
                   onPieceDrop: onDrop,

...
const getIconName = (cls: string): string => {
  const map: Record<string, string> = {
    "Brilliant": "splendid", "Great": "perfect", "Best": "best",
    "Excellent": "excellent", "Good": "okay", "Inaccuracy": "inaccuracy",
    "Mistake": "mistake", "Blunder": "blunder", "Book": "opening", "Forced": "forced",
  };
  return map[cls] || "okay";
};
...
export function ReviewPage({
  ...
  // fenAfter / fenBefore kept as-is (intentional feature)
  ...
  const lastMoveForHighlight = tab === "analysis" && sidelineFen ? null : lastMoveAfterPly;

  // FIX 3: Memoize chartData — only recomputes when analysis changes, not on every render
  const chartData = useMemo(() => {
    if (!analysis) return [];
    return [
      { ply: 0, eval: 0 },
      ...(analysis.moves || []).map((m: any) => ({
        ply: m.move_num,
  ...
  // FIX 1 & 2: abort fetch on unmount + clear analysis state to free memory
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if (e.name === "AbortError") return; // FIX 1: don't show error on intentional abort
    ...
    return () => {
      ctrl.abort();          // FIX 1: cancel in-flight fetch on unmount
      setAnalysis(null);     // FIX 2: free all 80 MoveAnalysis objects from memory
    };
  }, []);
  ...
  // Engine call on every currentPly change (kept as-is — intentional feature)
  useEffect(() => {
  ...
  const handlePieceDrop = useCallback((sourceSquare: string, targetSquare: string) => {
    ...
    return false;
  }, [sidelineFen, fenAfter]); // only recreated when these actually change
                        ...
                        (() => {
                          ...
                          return (
                            <div
                              ...
                              style={{ width: "12.5%", height: "12.5%", left: `${x * 12.5}%`, top: `${y * 12.5}%` }}
                            >
                              <img
